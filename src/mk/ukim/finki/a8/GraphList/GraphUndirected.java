...
public class GraphUndirected<T> {
    private int numNodes;
    private GraphNode<T>[] adjList;

    @SuppressWarnings("unchecked")
    public GraphUndirected(int numNodes) {
        this.numNodes = numNodes;
        this.adjList = (GraphNode<T>[]) new GraphNode[numNodes];
    }

    @SuppressWarnings("unchecked")
    public GraphUndirected(int numNodes, T[] adjList) {
        this.numNodes = numNodes;
        this.adjList = (GraphNode<T>[]) new GraphNode[numNodes];
        for (int i = 0; i < numNodes; ++i)
            this.adjList[i] = new GraphNode<>(i, adjList[i]);
    }
    ...
    public void addEdge(int x, int y) {
        ...
        adjList[y].addNeighbour(adjList[x]);
    ...
    private void BFSSearchNonrecursive(int startingNode, boolean[] visited) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < pom.neighbours.size(); i++) {
                GraphNode<T> tmp = pom.neighbours.get(i);
