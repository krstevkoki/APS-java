...
class OBHTPreveduvac<K extends Comparable<K>, V> implements Cloneable {
    private final MapEntry<K, V> former = new MapEntry<>(null, null);
    private MapEntry<K, V>[] buckets;

    @SuppressWarnings("unchecked")
    public OBHTPreveduvac(int m) {
    ...
    public OBHTPreveduvac<K, V> clone() {
        OBHTPreveduvac<K, V> copy = new OBHTPreveduvac<>(buckets.length);
    ...
    private int hash(K key) {
        String k = (String) key;
        return Math.abs(31 * k.length()) % buckets.length;
    }
